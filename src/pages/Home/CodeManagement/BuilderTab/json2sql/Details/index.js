...
const ModalDetail = (props) => {
	...
	const getData = async () => {
		try {
			const res = await json2SqlService.json2SqlCodeInfo({ id });
			if (res?.code === 10200) {
				const { data } = res || {};
	...
	const editCode = () => {
		form.validateFields().then(async (values) => {
			try {
				...
				const res = await json2SqlService.json2SqlCodeEdit(data);
				if (res?.code === 10200) {
		...
		return (
			<Form form={form} layout="vertical" initialValues={information}>
				...
				<div className="g-align-center">
					...
					<Form.Item name="sql" label="生成的代码" className="code">
	...
	return (
		<Modal
			...
			width={680}
			footer={null}
			wrapClassName="json2sqlModalRoot"
			maskClosable={false}
			destroyOnClose={true}
